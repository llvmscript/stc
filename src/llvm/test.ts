import ts, { BinaryExpression, ForStatement, FunctionDeclaration, Identifier, SyntaxKind, VariableDeclarationList } from 'typescript'
...
const gen_llvm = (ast: ts.NodeArray<ts.Node>, typeChecker: ts.TypeChecker) => {
    ...
    BinaryOperators[SyntaxKind.EqualsEqualsToken] = builder.CreateICmpEQ.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.EqualsEqualsToken] = builder.CreateFCmpOEQ.bind(builder);

    BinaryOperators[SyntaxKind.EqualsEqualsEqualsToken] = builder.CreateICmpEQ.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.EqualsEqualsEqualsToken] = builder.CreateFCmpOEQ.bind(builder);

    BinaryOperators[SyntaxKind.ExclamationEqualsToken] = builder.CreateICmpNE.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.ExclamationEqualsToken] = builder.CreateICmpNE.bind(builder);

    BinaryOperators[SyntaxKind.ExclamationEqualsEqualsToken] = builder.CreateICmpNE.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.ExclamationEqualsEqualsToken] = builder.CreateICmpNE.bind(builder);

    BinaryOperators[SyntaxKind.LessThanToken] = builder.CreateICmpSLT.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.LessThanToken] = builder.CreateFCmpOLT.bind(builder);

    BinaryOperators[SyntaxKind.LessThanEqualsToken] = builder.CreateICmpSLE.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.LessThanEqualsToken] = builder.CreateFCmpOLE.bind(builder);

    BinaryOperators[SyntaxKind.GreaterThanToken] = builder.CreateICmpSGT.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.GreaterThanToken] = builder.CreateFCmpOGT.bind(builder);

    BinaryOperators[SyntaxKind.GreaterThanEqualsToken] = builder.CreateICmpSGE.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.GreaterThanEqualsToken] = builder.CreateFCmpOGE.bind(builder);

    BinaryOperators[SyntaxKind.ExclamationToken] = builder.CreateNot.bind(builder);
    BinaryOperatorsFloat[SyntaxKind.ExclamationToken] = builder.CreateNot.bind(builder);

    // Unary!
    UnaryOperators[SyntaxKind.ExclamationToken] = builder.CreateNot.bind(builder);
    UnaryOperatorsFloat[SyntaxKind.ExclamationToken] = builder.CreateNot.bind(builder);
    ...
    ast.forEach(node => {
        switch (node.kind) {
            ...
            case SyntaxKind.FunctionDeclaration:
                create_function((node as FunctionDeclaration), builder, context, typeChecker, module)
    }
    })
    return module
}
...
const create_function = (node: FunctionDeclaration, builder: llvm.IRBuilder, context: llvm.LLVMContext, typeChecker: ts.TypeChecker, module: llvm.Module) => {
    ...
    body.forEachChild(node => {
        switch (node.kind) {
            case SyntaxKind.ForStatement:
                const start_block = create_for((node as ForStatement), builder, context, typeChecker, module);
...
const var_dec_list = (node: VariableDeclarationList, builder: llvm.IRBuilder, context: llvm.LLVMContext, typeChecker: ts.TypeChecker, module: llvm.Module) => {
    node.declarations.forEach(var_dec => {
        if (var_dec.name.kind === SyntaxKind.Identifier) {
            ...
            if (typeof value !== "undefined" && name && typeof name !== "undefined" && var_dec.initializer) {
                let type = typeChecker.typeToString(typeChecker.getTypeAtLocation(var_dec));
...
const handle_binary_expression = (node: ts.BinaryExpression, builder: llvm.IRBuilder, context: llvm.LLVMContext, typeChecker: ts.TypeChecker, module: llvm.Module) => {
    ...
    switch (node.left.kind) {
        case (SyntaxKind.Identifier):
            ...
            right_val_type = typeChecker.typeToString(typeChecker.getTypeAtLocation(node.left))
            const alloc_left = VARS[name]
            if (alloc_left) {
                left_val = builder.CreateLoad(alloc_left.getType().getPointerElementType(), alloc_left, name)
            } else {
                throw Error(`Variable ${name} does not exist.`)  // TODO: better message
            }
            break;
        case (SyntaxKind.NumericLiteral):
            let type = typeChecker.typeToString(typeChecker.getTypeAtLocation(node.left));

            if (!(type in TypeByName)) {
                type = "i64" // TODO: should this be a float? Probably but idk exactly how to do floats rn
            }
            right_val_type = type;
            left_val = TypeValueByName[type](parseInt((node.left as ts.NumericLiteral).text));
            break;
        default:
    ...
    switch (node.right.kind) {
        case (SyntaxKind.Identifier):
            ...
            if (alloc_right) {
                right_val = builder.CreateLoad(alloc_right.getType().getPointerElementType(), alloc_right, name)
            } else {
                throw Error(`Variable ${name} does not exist.`)  // TODO: better message
            }
            break;
        case (SyntaxKind.NumericLiteral):
            ...
            right_val = TypeValueByName[type](parseInt((node.right as ts.NumericLiteral).text));
            break;
        default:
    ...
    if (!llvm.Type.isSameType(right_val.getType(), left_val.getType())) {
        throw TypeError(`Cannot run operations on variables of two different types.`)  // TODO: better message
    }
    ...
    switch (right_val.getType().getTypeID()) {
        ...
        case llvm.Type.TypeID.IntegerTyID:
            break;
        default:
            throw Error('Sorry, you cannot run operations on things that arent floats or integers.');  // TODO: better message
    }

    if (right_val.getType().getTypeID() === llvm.Type.TypeID.IntegerTyID) {
        return BinaryOperators[node.operatorToken.kind](/*lhs*/ left_val, /*rhs*/ right_val)
    } else if (right_val.getType().getTypeID() === llvm.Type.TypeID.FloatTyID) {
        return BinaryOperatorsFloat[node.operatorToken.kind](/*lhs*/ left_val, /*rhs*/ right_val)
    }
}

const prefix_unary = (node: ts.PrefixUnaryExpression, builder: llvm.IRBuilder, context: llvm.LLVMContext, typeChecker: ts.TypeChecker, module: llvm.Module) => {
    let val: llvm.Value;
    ...
    switch (node.operand.kind) {
        case (SyntaxKind.Identifier):
            ...
            if (alloc) {
                val = builder.CreateLoad(alloc.getType().getPointerElementType(), alloc, name)
                var_alloc = alloc;
            } else {
                throw Error(`Variable ${name} does not exist.`)  // TODO: better message
            }
            break;
        case (SyntaxKind.NumericLiteral):
            if (node.operator === SyntaxKind.MinusMinusToken || node.operator === SyntaxKind.PlusPlusToken) {
                throw SyntaxError("Cannot increment or decrement number")
            }
            let type = typeChecker.typeToString(typeChecker.getTypeAtLocation(node.operand));

            if (!(type in TypeByName)) {
                type = "i64" // TODO: should this be a float? Probably but idk exactly how to do floats rn
            }
            val = TypeValueByName[type](parseInt((node.operand as ts.NumericLiteral).text));
            break;
        default:
            throw Error('Not implemented')  // TODO: better message
    }
 ...
 */
export const create_for = (node: ForStatement, builder: llvm.IRBuilder, context: llvm.LLVMContext, typeChecker: ts.TypeChecker, module: llvm.Module) => {
    ...
    if (node.initializer) {
        switch (node.initializer.kind) {
            case SyntaxKind.VariableDeclarationList:
                var_dec_list((node.initializer as unknown as ts.VariableDeclarationList), builder, context, typeChecker, module);
    ...
    if (node.condition) {
        switch (node.condition.kind) {
            ...
            case SyntaxKind.BinaryExpression:
                condCheck = handle_binary_expression((node.condition as BinaryExpression), builder ,context, typeChecker, module)
        }
    }
    if (!condCheck) {
    ...
    if (node.incrementor) {
        switch (node.incrementor.kind) {
            ...
            case SyntaxKind.PrefixUnaryExpression: // eg ++i or --i
                // TODO
                prefix_unary(node.incrementor as ts.PrefixUnaryExpression, builder,context, typeChecker, module)
